...
function warnings(value: unknown): string[] {
  ...
  const result = (value as { warnings?: unknown }).warnings;
...
export default async function ExportJobDetailPage({ params }: PageProps) {
  ...
  const job = await db.exportJob.findUnique({
    ...
    include: {
      ...
      items: { orderBy: { createdAt: "asc" }, include: { product: { select: { id: true, titleSource: true, titleEdited: true, sku: true, barcode: true } } } },
    },
  });
  if (!job) notFound();
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">Export Job</p><h1 className="mt-1 break-all text-2xl font-bold text-gray-900">{job.id}</h1><p className="mt-2 text-sm text-gray-500">Marketplace: {job.targetMarketplace} · Status: {job.status} · Account: {job.marketplaceAccount?.name ?? "Environment variables"}</p><p className="mt-2 text-sm font-medium text-gray-700">{completed} completed · {failed} failed · {job.items.length} total</p></div>
            <div className="flex flex-wrap gap-2"><RefreshButton />{job.targetMarketplace === "shopify" ? <RunShopifyExportButton jobId={job.id} disabled={job.status === "running"} /> : null}</div>
          </div>
        </section>
        <section className="space-y-3">
          {job.items.map((item) => {
            const itemWarnings = warnings(item.responsePayload);
            return <article key={item.id} className="rounded-[24px] border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between"><div><Link href={`/products/${item.product.id}`} className="font-semibold text-gray-900 hover:underline">{item.product.titleEdited ?? item.product.titleSource}</Link><p className="mt-1 text-sm text-gray-500">SKU: {item.product.sku ?? "-"} · External ID: {item.targetExternalId ?? "-"}</p></div><span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-700">{item.status}</span></div>
